class Report < ApplicationRecord
	...
	def num_issues(val='num')
		...
		@issues = 0
		@issue_text = ['leaks','wires','belts','oil','coolant','battery','transmission','noise','guages','switches','horn','defrosters', 'wipers','stop_arm','mirrors','brakes','service_door','emergency_equipment','first_aid','stairs','interior','floor','emergency_door','lights','front_right_wheel','windshield','front_left_tire','exhaust','left_side','left_rear_tire','rear','tail_pipe','right_rear_tire','right_side', 'drivers_seat','directionals','parking_brake','clutch','steering','wheelchair_lift','condition']
		@issue_attributes = [self.leaks, self.wires, self.belts, self.oil, self.coolant, self.battery, self.transmission, self.noise, self.guages, self.switches, self.horn, self.defrosters, self.wipers, self.stop_arm, self.mirrors, self.brakes, self.service_door, self.emergency_equipment, self.first_aid, self.stairs, self.interior, self.floor, self.emergency_door, self.lights, self.front_right_wheel, self.windshield, self.front_left_tire, self.exhaust, self.left_side, self.left_rear_tire, self.rear, self.tail_pipe, self.right_rear_tire, self.right_side, self.drivers_seat,self.directionals, self.parking_brake, self.clutch, self.steering, self.wheelchair_lift,self.condition]
